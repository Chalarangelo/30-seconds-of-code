 ...
 */
export const getRichConfigs = (configs, langData) =>
  configs.map(cfg => {
    const {
...
export const loadContentConfigs = (inPath, boundLog) => {
  boundLog(
    `Loading individual configuration files in ${path.resolve(
      inPath,
      'configs'
    )}`,
    'info'
  ...
  if (global.yild.configs) {
    ...
    const rawConfigs = JSONParser.fromGlob(`${inPath}/configs/*.json`);
    const langData = rawConfigs
      .filter(
        cfg =>
          cfg.language && cfg.language.long && cfg.theme && cfg.theme.iconName
      )
      .map(cfg => ({
