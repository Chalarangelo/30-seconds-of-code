...
export const compileListingPages = async (
  ...
  for (let [i, chunk] of chunks.entries()) {
    ...
    const isTopLevelListing =
      (context.listingType === 'language' || context.listingType === 'blog') &&
      isPopularityOrdered;
...
export const compileListingData = async (snippetIndex, listingMetas) => {
  ...
  const mainContextCustomizer = order => {
    return {
      listingSublinks: mainListingSublinks.map(l => ({
        ...l,
  ...
  for (let listingMeta of listingMetas) {
    ...
    const snippetIndexName =
      snippetIndex.find(s => s.slug.startsWith(`${slugPrefix}`)).language
        .long || '';
    const snippetIndexSlugData = snippetIndex.filter(
      s =>
    ...
    const expertiseSlugChunks = chunk(
      transformedSlugChunks.sort((a, b) =>
    ...
    await compileListingPagesWithOrderOptions(
      {
        listingName: listingMeta.blog
          ? literals.blog
          : literals.codelang(snippetIndexName),
        listingTitle: listingMeta.blog
          ? literals.blog
          : literals.codelang(snippetIndexName),
        listingType: listingMeta.blog ? 'blog' : 'language',
        pageDescription: literals.pageDescription(
          listingMeta.blog ? 'blog' : 'language',
          {
    ...
    for (let tagPrefix of snippetIndexTagPrefixes) {
      const currentTag = listingMeta.tags.find(tag => tag === tagPrefix);
      // Determine slug prefix and relevant information, create chunks from data
      // for each of the ordering options
      const snippetIndexTagData = snippetIndex.filter(
        s =>
          (s.slug.startsWith(`${slugPrefix}`) &&
            s.tags.all.find(
              t => t.toLowerCase() === tagPrefix.toLowerCase()
            )) ||
          (s.blog &&
            s.tags.all.find(
              t => t.toLowerCase() === snippetIndexName.toLowerCase()
      ...
      const alphabeticalTagChunks = chunk(
        transformedTagChunks.sort((a, b) => a.title.localeCompare(b.title)),
        CARDS_PER_PAGE
      );
      const expertiseTagChunks = chunk(
        transformedTagChunks.sort((a, b) =>
          a.expertise === b.expertise
            ? a.title.localeCompare(b.title)
            : !a.expertise
            ? 1
        ...
        return {
          listingSublinks: listingMeta.blog
            ...
            : [
                ...
                ...tag,
                selected: tag.url.indexOf(`/t/${tagPrefix}/`) !== -1,
              })),
        };
      };
      // Create the listing pages with the order options provided
