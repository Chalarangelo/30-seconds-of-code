...
const determineRecommendedSnippets = (snippetNodes, snippetContext) => {
  ...
  const language = (snippetContext.blog
    ? Tag.determineLanguage(snippetContext.tags.all).long
    : snippetContext.language.long
  ).toLowerCase();
  const primaryTag = (snippetContext.blog
    ? Tag.stripLanguage(snippetContext.tags.all)[0]
    : snippetContext.tags.primary
  ).toLowerCase();
  ...
  return snippetNodes
    .map(v => {
      ...
      if (
        v.id !== snippetContext.id &&
        v.titleSlug !== snippetContext.titleSlug &&
        (!snippetContext.isListed || v.isListed)
      ) {
        ...
        const languageScore =
          (v.language && v.language.long === language
            ? 1
            : v.tags.all.find(t => t.toLowerCase() === language)
            ? 0.25
        ...
        const primaryTagScore =
          (v.tags.primary.toLowerCase() === primaryTag
            ? 1
            : v.tags.all.find(t => t.toLowerCase() === primaryTag)
            ? 0.5
