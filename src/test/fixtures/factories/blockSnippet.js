...
const {
  ...
  blog: blogConfig,
  react: reactConfig,
} = ContentConfigFactory.create('ContentConfigPresets');
...
factory
  ...
  .trait('unlisted', { unlisted: true, fileName: 'none.md' })
  .trait('css', {
    body:
  ...
  .trait('blog', {
    ...
    cover: 'blog_images/an-image.jpg',
    excerpt:
  ...
  .trait('future', {
    firstSeen: '2240438400000',
    lastUpdated: '2240438400000',
  });

factory
  .define(
    'Snippet',
    () => new Snippet(factory.create('RawSnippet'), dartConfig)
  )
  .trait(
    'css',
    () => new Snippet(factory.create('RawSnippet', 'css'), cssConfig)
  )
  .trait(
    'blog',
    () => new Snippet(factory.create('RawSnippet', 'blog'), blogConfig)
  )
  .trait('react', () => new Snippet(factory.create('RawSnippet'), reactConfig));

let snippets = false;

factory.define('SnippetPresets', () => {
  snippets = !snippets
    ? {
        snippet: factory.create('Snippet', 'react'),
        blogSnippet: factory.create('Snippet', 'blog'),
        unlistedSnippet: factory.create('Snippet'),
        cssSnippet: factory.create('Snippet', 'css'),
      }
    : snippets;

  delete snippets.blogSnippet.code;
