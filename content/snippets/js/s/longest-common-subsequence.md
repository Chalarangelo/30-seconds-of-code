...
Cool visualization, huh? _But how does it work?_ Like I said, we break the problem down into smaller subproblems. The table above shows the **lengths of the longest common subsequences of the prefixes** of the two strings. The **rows** represent the characters of the first string, and the **columns** represent the characters of the second string. The **cell** at row `i` and column `j` contains the length of the longest common subsequence of the prefixes `string1.slice(0, i)` and `string2.slice(0, j)`. The **empty string** `ε` represents the empty prefix.
