...
var UNKNOWN_ARRAY_EXPRESSION = {
    ...
    hasEffectsWhenCalledAtPath: function (path$$1, callOptions, options) {
        if (path$$1.length === 1) {
            return hasMemberEffectWhenCalled(arrayMembers, path$$1[0], callOptions, options);
        }
        return true;
    },
...
function hasMemberEffectWhenCalled(members, memberName, callOptions, options) {
    if (isUnknownKey(memberName) || !members[memberName])
        return true;
...
var ReassignedPathTracker = /** @class */ (function () {
    function ReassignedPathTracker() {
        ...
        this.unknownReassignedSubPath = false;
    ...
    ReassignedPathTracker.prototype.isReassigned = function (path$$1) {
        ...
        return (this.unknownReassignedSubPath ||
    ...
    ReassignedPathTracker.prototype.reassignPath = function (path$$1) {
        ...
        if (path$$1.length === 0) {
            ...
            this.reassignSubPath(path$$1);
        }
    };
    ReassignedPathTracker.prototype.reassignSubPath = function (path$$1) {
        if (this.unknownReassignedSubPath)
            return;
        var subPath = path$$1[0], remainingPath = path$$1.slice(1);
        if (subPath === UNKNOWN_KEY) {
            ...
            if (!this.subPaths.has(subPath)) {
                this.subPaths.set(subPath, new ReassignedPathTracker());
            }
            this.subPaths.get(subPath).reassignPath(remainingPath);
        }
    };
    ReassignedPathTracker.prototype.someReassignedPath = function (path$$1, callback) {
        return this.reassigned
            ? callback(path$$1, UNKNOWN_EXPRESSION)
            : path$$1.length >= 1 && this.onSubPathIfReassigned(path$$1, callback);
    };
    ReassignedPathTracker.prototype.onSubPathIfReassigned = function (path$$1, callback) {
        var subPath = path$$1[0], remainingPath = path$$1.slice(1);
        return this.unknownReassignedSubPath || subPath === UNKNOWN_KEY
            ? callback(remainingPath, UNKNOWN_EXPRESSION)
            : this.subPaths.has(subPath) &&
                this.subPaths.get(subPath).someReassignedPath(remainingPath, callback);
    };
...
var VariableReassignmentTracker = /** @class */ (function () {
    function VariableReassignmentTracker(initialExpression) {
        this.reassignedPathTracker = new ReassignedPathTracker();
        this.initialExpression = initialExpression;
    }
    VariableReassignmentTracker.prototype.reassignPath = function (path$$1, options) {
        if (path$$1.length > 0) {
            this.initialExpression && this.initialExpression.reassignPath(path$$1, options);
        }
        this.reassignedPathTracker.reassignPath(path$$1);
    };
    VariableReassignmentTracker.prototype.forEachAtPath = function (path$$1, callback) {
        this.initialExpression && callback(path$$1, this.initialExpression);
    };
    VariableReassignmentTracker.prototype.someAtPath = function (path$$1, predicateFunction) {
        return (this.reassignedPathTracker.someReassignedPath(path$$1, predicateFunction) ||
            (this.initialExpression && predicateFunction(path$$1, this.initialExpression)));
    };
...
var LocalVariable = /** @class */ (function (_super) {
    ...
    LocalVariable.prototype.include = function () {
        if (!this.included) {
            this.included = true;
            this.declarations.forEach(function (node) {
    ...
    ModuleScope.prototype.deshadow = function (names, children) {
        ...
        for (var _i = 0, _a = Object.keys(this.context.imports); _i < _a.length; _i++) {
            ...
            if (importDescription.name !== '*') {
                ...
                if (importDescription.name !== 'default' &&
                    importDescription.specifier.imported.name !==
                        importDescription.specifier.local.name) {
                    localNames.add(importDescription.specifier.imported.name);
                }
            }
        }
...
var FunctionNode = /** @class */ (function (_super) {
    ...
    FunctionNode.prototype.bind = function () {
...
var Identifier = /** @class */ (function (_super) {
    ...
    Identifier.prototype.include = function () {
        if (!this.included) {
            this.included = true;
    ...
    Identifier.prototype.render = function (code, options, _a) {
...
var ExportDefaultDeclaration = /** @class */ (function (_super) {
    ...
    ExportDefaultDeclaration.prototype.bind = function () {
        _super.prototype.bind.call(this);
...
var ArrayPattern = /** @class */ (function (_super) {
    ...
    ArrayPattern.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
        ...
        for (var _i = 0, _a = this.elements; _i < _a.length; _i++) {
            var element = _a[_i];
            if (element !== null && element.hasEffectsWhenAssignedAtPath([], options))
                return true;
        }
        return false;
    };
...
var BlockStatement = /** @class */ (function (_super) {
    ...
    BlockStatement.prototype.include = function () {
        this.included = true;
        for (var _i = 0, _a = this.body; _i < _a.length; _i++) {
            var node = _a[_i];
            if (node.shouldBeIncluded())
...
var ArrowFunctionExpression = /** @class */ (function (_super) {
    ...
    ArrowFunctionExpression.prototype.bind = function () {
        _super.prototype.bind.call(this);
...
var CallExpression = /** @class */ (function (_super) {
    ...
    CallExpression.prototype.forEachReturnExpressionWhenCalledAtPath = function (path$$1, callOptions, callback, options) {
        this.callee.forEachReturnExpressionWhenCalledAtPath([], this.callOptions, function (innerOptions) { return function (node) {
    ...
    CallExpression.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
        ...
        return (path$$1.length > 0 &&
            ...
            this.callee.someReturnExpressionWhenCalledAtPath([], this.callOptions, function (innerOptions) { return function (node) {
    ...
    CallExpression.prototype.someReturnExpressionWhenCalledAtPath = function (path$$1, callOptions, predicateFunction, options) {
        return this.callee.someReturnExpressionWhenCalledAtPath([], this.callOptions, function (innerOptions) { return function (node) {
...
var ForInStatement = /** @class */ (function (_super) {
    ...
    ForInStatement.prototype.include = function () {
        this.included = true;
...
var ForOfStatement = /** @class */ (function (_super) {
    ...
    ForOfStatement.prototype.include = function () {
        this.included = true;
        this.left.includeWithAllDeclaredVariables();
        this.right.include();
        this.body.include();
...
var Literal = /** @class */ (function (_super) {
    ...
    Literal.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
        if (path$$1.length === 1) {
            return hasMemberEffectWhenCalled(this.members, path$$1[0], callOptions, options);
        }
        return true;
    };
    ...
    Literal.prototype.someReturnExpressionWhenCalledAtPath = function (path$$1, callOptions, predicateFunction, options) {
        if (path$$1.length === 1) {
...
var MemberExpression = /** @class */ (function (_super) {
    ...
    MemberExpression.prototype.include = function () {
        if (!this.included) {
            this.included = true;
            if (this.variable !== null && !this.variable.included) {
                this.variable.include();
    ...
    MemberExpression.prototype.render = function (code, options, _a) {
        var _b = _a === void 0 ? BLANK : _a, renderedParentType = _b.renderedParentType, isCalleeOfRenderedParent = _b.isCalleeOfRenderedParent;
        var isCalleeOfDifferentParent = renderedParentType === NodeType.CallExpression && isCalleeOfRenderedParent;
...
var ObjectExpression = /** @class */ (function (_super) {
    ...
    ObjectExpression.prototype.forEachReturnExpressionWhenCalledAtPath = function (path$$1, callOptions, callback, options) {
        if (path$$1.length === 0)
            return;
        var _a = this.getPossiblePropertiesWithName(path$$1[0], PROPERTY_KINDS_READ), properties = _a.properties, hasCertainHit = _a.hasCertainHit;
        if (hasCertainHit) {
    ...
    ObjectExpression.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
        ...
        for (var _i = 0, properties_2 = properties; _i < properties_2.length; _i++) {
            ...
            if (property.hasEffectsWhenAccessedAtPath(path$$1.slice(1), options))
                return true;
        }
        return false;
    };
    ObjectExpression.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
        if (path$$1.length === 0)
            return false;
        var _a = this.getPossiblePropertiesWithName(path$$1[0], path$$1.length === 1 ? PROPERTY_KINDS_WRITE : PROPERTY_KINDS_READ), properties = _a.properties, hasCertainHit = _a.hasCertainHit;
        if (path$$1.length > 1 && !hasCertainHit)
            return true;
    ...
    ObjectExpression.prototype.hasEffectsWhenCalledAtPath = function (path$$1, callOptions, options) {
        if (path$$1.length === 0)
            return true;
        var subPath = path$$1[0];
        if (path$$1.length === 1 && !isUnknownKey(subPath) && objectMembers[subPath]) {
            return false;
        }
        var _a = this.getPossiblePropertiesWithName(path$$1[0], PROPERTY_KINDS_READ), properties = _a.properties, hasCertainHit = _a.hasCertainHit;
        if (!hasCertainHit)
        ...
        for (var _i = 0, properties_4 = properties; _i < properties_4.length; _i++) {
            ...
            if (property.hasEffectsWhenCalledAtPath(path$$1.slice(1), callOptions, options))
                return true;
        }
        return false;
    };
    ...
    ObjectExpression.prototype.someReturnExpressionWhenCalledAtPath = function (path$$1, callOptions, predicateFunction, options) {
        ...
        var _a = this.getPossiblePropertiesWithName(subPath, PROPERTY_KINDS_READ), properties = _a.properties, hasCertainHit = _a.hasCertainHit;
        if (!hasCertainHit)
            return true;
        for (var _i = 0, properties_6 = properties; _i < properties_6.length; _i++) {
            var property = properties_6[_i];
            if (property.someReturnExpressionWhenCalledAtPath(path$$1.slice(1), callOptions, predicateFunction, options))
                return true;
        }
        return false;
    };
    ObjectExpression.prototype.getPossiblePropertiesWithName = function (name, kinds) {
        if (name === UNKNOWN_KEY) {
...
var ObjectPattern = /** @class */ (function (_super) {
    ...
    ObjectPattern.prototype.hasEffectsWhenAssignedAtPath = function (path$$1, options) {
        ...
        for (var _i = 0, _a = this.properties; _i < _a.length; _i++) {
            var property = _a[_i];
            if (property.hasEffectsWhenAssignedAtPath([], options))
                return true;
        }
        return false;
    };
...
var Property = /** @class */ (function (_super) {
    ...
    Property.prototype.declare = function (kind, _init) {
    ...
    Property.prototype.forEachReturnExpressionWhenCalledAtPath = function (path$$1, callOptions, callback, options) {
        if (this.kind === 'get') {
            this.value.forEachReturnExpressionWhenCalledAtPath([], this.accessorCallOptions, function (innerOptions) { return function (node) {
                return node.forEachReturnExpressionWhenCalledAtPath(path$$1, callOptions, callback, innerOptions);
            }; }, options);
        }
        else {
    ...
    Property.prototype.hasEffectsWhenAccessedAtPath = function (path$$1, options) {
        ...
        if (this.kind === 'get') {
            return (this.value.hasEffectsWhenCalledAtPath([], this.accessorCallOptions, options.getHasEffectsWhenCalledOptions()) ||
                (!options.hasReturnExpressionBeenAccessedAtPath(path$$1, this) &&
                    this.value.someReturnExpressionWhenCalledAtPath([], this.accessorCallOptions, function (innerOptions) { return function (node) {
                        return node.hasEffectsWhenAccessedAtPath(path$$1, innerOptions.addAccessedReturnExpressionAtPath(path$$1, _this));
                    }; }, options)));
        }
...
var VariableDeclaration = /** @class */ (function (_super) {
    ...
    VariableDeclaration.prototype.include = function () {
        this.included = true;
        for (var _i = 0, _a = this.declarations; _i < _a.length; _i++) {
            var declarator = _a[_i];
...
var Module = /** @class */ (function () {
    ...
    Module.prototype.getAndCreateNamespace = function () {
    ...
    Module.prototype.traceVariable = function (name) {
        ...
        if (name in this.imports) {
            ...
            if (!otherModule.isExternal && importDeclaration.name === '*') {
                return otherModule.getAndCreateNamespace();
            }
            var declaration = otherModule.traceExport(importDeclaration.name);
            if (!declaration) {
                this.graph.handleMissingExport(importDeclaration.name, this, otherModule.id, importDeclaration.specifier.start);
            }
            return declaration;
        }
        return null;
...
var ExternalModule = /** @class */ (function () {
    ...
    ExternalModule.prototype.setRenderPath = function (options, inputPath) {
        if (typeof options.paths === 'function') {
            var outPath = options.paths(this.id);
            if (outPath) {
        ...
        if (isAbsolute(this.id)) {
            var outDir = void 0;
            if (options.dir)
                outDir = path.resolve(options.dir);
            else if (options.file)
                outDir = path.dirname(path.resolve(options.file));
            else
                outDir = path.dirname(inputPath);
            var relativeToEntry = normalize(path.relative(outDir, this.id));
...
function amd(magicString, _a, options) {
    var graph = _a.graph, exportMode = _a.exportMode, indentString = _a.indentString, intro = _a.intro, outro = _a.outro, dynamicImport = _a.dynamicImport, dependencies = _a.dependencies, exports = _a.exports, isEntryModuleFacade = _a.isEntryModuleFacade;
...
function cjs(magicString, _a, options) {
    var graph = _a.graph, isEntryModuleFacade = _a.isEntryModuleFacade, exportMode = _a.exportMode, intro = _a.intro, outro = _a.outro, dependencies = _a.dependencies, exports = _a.exports;
    intro =
...
function iife(magicString, _a, options) {
    var graph = _a.graph, exportMode = _a.exportMode, indentString = _a.indentString, intro = _a.intro, outro = _a.outro, dependencies = _a.dependencies, exports = _a.exports;
...
var require$$1 = ( inherits_browser$1 && inherits_browser ) || inherits_browser$1;
...
var assert$1 = ( minimalisticAssert$1 && minimalisticAssert ) || minimalisticAssert$1;
...
var common$4 = ( common$1 && common ) || common$1;

var shaCommon = ( common$3 && common$2 ) || common$3;
...
var Chunk$1 = /** @class */ (function () {
    ...
    Chunk.prototype.inlineDeepModuleDependencies = function () {
        var _this = this;
        // if an entry point facade, inline the execution list to avoid loading latency
        if (this.isEntryModuleFacade) {
            var inlineDeepChunkDependencies_1 = function (dep) {
                if (dep === _this || _this.dependencies.indexOf(dep) !== -1) {
                    return;
                }
                _this.dependencies.push(dep);
                if (dep instanceof Chunk) {
                    dep.dependencies.forEach(inlineDeepChunkDependencies_1);
                }
            };
    ...
    Chunk.prototype.linkFacade = function (entryFacade) {
        ...
        for (var _i = 0, _a = entryFacade.getAllExports(); _i < _a.length; _i++) {
            var exportName = _a[_i];
            var tracedVariable = entryFacade.traceVariable(exportName);
    ...
    Chunk.prototype.link = function () {
        ...
        for (var _i = 0, _a = this.orderedModules; _i < _a.length; _i++) {
            ...
            for (var _d = 0, _e = Object.keys(module.imports); _d < _e.length; _d++) {
                var importName = _e[_d];
        ...
        if (mangle) {
            for (var _i = 0, exportedVariables_1 = exportedVariables; _i < exportedVariables_1.length; _i++) {
                ...
                if (safeExportName.charCodeAt(0) === 49 /* '1' */) {
                    i += 9 * Math.pow(36, (safeExportName.length - 1));
                    safeExportName = i.toString(36);
                }
    ...
    Chunk.prototype.prepareDynamicImports = function (_a) {
        ...
        for (var _i = 0, _b = this.orderedModules; _i < _b.length; _i++) {
            var module = _b[_i];
            for (var i = 0; i < module.dynamicImportResolutions.length; i++) {
                ...
                if (resolution instanceof Module) {
                    ...
                    if (resolution.chunk === this) {
                        var namespace = resolution.getAndCreateNamespace();
                        namespace.include();
    ...
    Chunk.prototype.setIdentifierRenderResolutions = function (options) {
        ...
        Array.from(this.imports.entries()).forEach(function (_a) {
            ...
            if (module instanceof ExternalModule) {
                ...
                safeName = module.chunk.name + "." + module.chunk.getVariableExportName(variable);
            }
        ...
        this.orderedModules.forEach(function (module) {
            ...
            var namespace = module.getAndCreateNamespace();
    ...
    Chunk.prototype.getChunkDependencyDeclarations = function (options) {
        var _this = this;
        ...
        this.dependencies.forEach(function (dep) {
            ...
            if (dep instanceof ExternalModule) {
                ...
                if (options.format === 'umd' || options.format === 'iife') {
                    globalName = getGlobalName(dep, options.globals, _this.graph, exportsNames || exportsNamespace || exportsDefault);
                }
            }
    ...
    Chunk.prototype.preRender = function (options) {
        ...
        this.renderedSourceLength = undefined;
        if (this.getExportNames().length === 0 &&
     ...
     */
    Chunk.prototype.merge = function (chunk, chunkList, options) {
        var _this = this;
    ...
    Chunk.prototype.render = function (options, addons) {
        ...
        var exportMode = this.isEntryModuleFacade ? getExportMode(this, options) : 'named';
        ...
        for (var i = 0; i < this.dependencies.length; i++) {
            ...
            if (dep instanceof ExternalModule)
                continue;
            var relPath = normalize(path.relative(path.dirname(this.id), dep.id));
            if (!relPath.startsWith('../'))
                relPath = './' + relPath;
 ...
 *
 */
function optimizeChunks(chunks, options, CHUNK_GROUPING_SIZE) {
    ...
    try {
        ...
        return graph_1
            ...
            function generate(rawOutputOptions) {
                ...
                timeStart('GENERATE', 1);
                var generated = {};
                var preserveModulesBase;
                if (inputOptions_1.experimentalPreserveModules)
                    preserveModulesBase = commondir(chunks.map(function (chunk) { return chunk.entryModule.id; }));
                ...
                var promise = createAddons(graph_1, outputOptions)
                    ...
                    return (Promise.resolve()
                        ...
                        if (!optimized && inputOptions_1.optimizeChunks) {
                            ...
                            optimizeChunks(chunks, outputOptions, inputOptions_1.chunkGroupingSize);
                            optimized = true;
                        }
...
var filenameRegex$1 = /*#__PURE__*/Object.freeze({
    default: filenameRegex,
    __moduleExports: filenameRegex
});
...
var arrFlatten$1 = /*#__PURE__*/Object.freeze({
    default: arrFlatten,
    __moduleExports: arrFlatten
});

var flatten = ( arrFlatten$1 && arrFlatten ) || arrFlatten$1;
...
var arrDiff$1 = /*#__PURE__*/Object.freeze({
    default: arrDiff,
    __moduleExports: arrDiff
});
...
var arrayUnique$1 = /*#__PURE__*/Object.freeze({
    default: arrayUnique,
    __moduleExports: arrayUnique
});
...
var isArray$1 = ( isarray$1 && isarray ) || isarray$1;
...
var isNumber$3 = /*#__PURE__*/Object.freeze({
    default: isNumber$2,
    __moduleExports: isNumber$2
});
...
var kindOf$5 = /*#__PURE__*/Object.freeze({
    default: kindOf$4,
    __moduleExports: kindOf$4
});

var isNumber$4 = ( isNumber$3 && isNumber$2 ) || isNumber$3;

var typeOf$2 = ( kindOf$5 && kindOf$4 ) || kindOf$5;
...
function randomatic(pattern, length, options) {
  ...
  if (arguments.length === 1) {
    ...
    } else if (isNumber$4(pattern)) {
      options = {}; length = pattern; pattern = '*';
    }
  }

  if (typeOf$2(length) === 'object' && length.hasOwnProperty('chars')) {
    options = length;
...
var randomatic$1 = /*#__PURE__*/Object.freeze({
    default: randomatic_1,
    __moduleExports: randomatic_1
});
...
var repeatElement$1 = /*#__PURE__*/Object.freeze({
    default: repeatElement,
    __moduleExports: repeatElement
});

var isObject$1 = ( isobject$1 && isobject ) || isobject$1;

var isNumber$5 = ( isNumber$1 && isNumber ) || isNumber$1;

var randomize = ( randomatic$1 && randomatic_1 ) || randomatic$1;

var repeatStr = ( repeatString$1 && repeatString ) || repeatString$1;

var repeat$1 = ( repeatElement$1 && repeatElement ) || repeatElement$1;
...
function fillRange(a, b, step, options, fn) {
  ...
  // validate arguments
  var isNumA = isNumber$5(zeros(a));
  var isNumB = isNumber$5(zeros(b));
...
var fillRange$1 = /*#__PURE__*/Object.freeze({
    default: fillRange_1,
    __moduleExports: fillRange_1
});

var fill = ( fillRange$1 && fillRange_1 ) || fillRange$1;
...
var expandRange$1 = /*#__PURE__*/Object.freeze({
    default: expandRange,
    __moduleExports: expandRange
});
...
var preserve$1 = /*#__PURE__*/Object.freeze({
    default: preserve,
    __moduleExports: preserve,
    before: before,
    after: after
});

var expand = ( expandRange$1 && expandRange ) || expandRange$1;

var tokens = ( preserve$1 && preserve ) || preserve$1;
  ...
  while (len--) {
    ...
    if (idx === -1) {
      ...
      if (!!ele && opts.nodupes !== false) {
        ...
        arr.push.apply(arr, repeat$1(ele, num));
      }
    }
  }
  return arr;
...
var braces$1 = /*#__PURE__*/Object.freeze({
    default: braces_1,
    __moduleExports: braces_1
});
...
var isPosixBracket$1 = /*#__PURE__*/Object.freeze({
    default: isPosixBracket,
    __moduleExports: isPosixBracket
});

var isPosixBracket$2 = ( isPosixBracket$1 && isPosixBracket ) || isPosixBracket$1;
...
var expandBrackets$1 = /*#__PURE__*/Object.freeze({
    default: expandBrackets,
    __moduleExports: expandBrackets
});
...
var isExtglob$1 = /*#__PURE__*/Object.freeze({
    default: isExtglob,
    __moduleExports: isExtglob
});

var extglob = ( isExtglob$1 && isExtglob ) || isExtglob$1;
...
var extglob$2 = /*#__PURE__*/Object.freeze({
    default: extglob_1,
    __moduleExports: extglob_1
});
...
var isGlob$1 = /*#__PURE__*/Object.freeze({
    default: isGlob,
    __moduleExports: isGlob
});
...
var removeTrailingSeparator$1 = /*#__PURE__*/Object.freeze({
    default: removeTrailingSeparator,
    __moduleExports: removeTrailingSeparator
});

var removeTrailingSeparator$2 = ( removeTrailingSeparator$1 && removeTrailingSeparator ) || removeTrailingSeparator$1;
...
var normalizePath = function normalizePath(str, stripTrailing) {
  ...
  if (stripTrailing !== false) {
    str = removeTrailingSeparator$2(str);
  }
  return str;
};

var normalizePath$1 = /*#__PURE__*/Object.freeze({
    default: normalizePath,
    __moduleExports: normalizePath
});
...
var forIn$1 = /*#__PURE__*/Object.freeze({
    default: forIn,
    __moduleExports: forIn
});

var forIn$2 = ( forIn$1 && forIn ) || forIn$1;

var hasOwn = Object.prototype.hasOwnProperty;

var forOwn = function forOwn(obj, fn, thisArg) {
  forIn$2(obj, function(val, key) {
...
var forOwn$1 = /*#__PURE__*/Object.freeze({
    default: forOwn,
    __moduleExports: forOwn
});

var isObject$2 = ( isExtendable$1 && isExtendable ) || isExtendable$1;

var forOwn$2 = ( forOwn$1 && forOwn ) || forOwn$1;
...
var object_omit = function omit(obj, keys) {
  ...
  forOwn$2(obj, function(value, key) {
...
var object_omit$1 = /*#__PURE__*/Object.freeze({
    default: object_omit,
    __moduleExports: object_omit
});

var isglob = ( isGlob$1 && isGlob ) || isGlob$1;
...
var globParent$1 = /*#__PURE__*/Object.freeze({
    default: globParent,
    __moduleExports: globParent
});

var parent = ( globParent$1 && globParent ) || globParent$1;
...
var globBase$1 = /*#__PURE__*/Object.freeze({
    default: globBase,
    __moduleExports: globBase
});
...
var isDotfile$1 = /*#__PURE__*/Object.freeze({
    default: isDotfile,
    __moduleExports: isDotfile
});

var findBase = ( globBase$1 && globBase ) || globBase$1;

var dotfile = ( isDotfile$1 && isDotfile ) || isDotfile$1;
...
var parseGlob$1 = /*#__PURE__*/Object.freeze({
    default: parseGlob,
    __moduleExports: parseGlob,
    cache: parseGlob_1
});
...
var isEqualShallow = function isEqual(a, b) {
  ...
  for (key in b) {
    numKeysB++;
    if (!isPrimitive$2(b[key]) || !a.hasOwnProperty(key) || (a[key] !== b[key])) {
      return false;
    }
  }
  for (key in a) {
...
var isEqualShallow$1 = /*#__PURE__*/Object.freeze({
    default: isEqualShallow,
    __moduleExports: isEqualShallow
});

var equal = ( isEqualShallow$1 && isEqualShallow ) || isEqualShallow$1;
...
var regexCache$1 = /*#__PURE__*/Object.freeze({
    default: regexCache_1,
    __moduleExports: regexCache_1,
    cache: cache_1,
    basic: basic_1
});

var fileRe = ( filenameRegex$1 && filenameRegex ) || filenameRegex$1;

var require$$0 = ( arrDiff$1 && arrDiff ) || arrDiff$1;

var require$$1$1 = ( arrayUnique$1 && arrayUnique ) || arrayUnique$1;

var require$$2 = ( braces$1 && braces_1 ) || braces$1;

var require$$3 = ( expandBrackets$1 && expandBrackets ) || expandBrackets$1;

var require$$4 = ( extglob$2 && extglob_1 ) || extglob$2;

var require$$8 = ( normalizePath$1 && normalizePath ) || normalizePath$1;

var require$$9 = ( object_omit$1 && object_omit ) || object_omit$1;

var require$$10 = ( parseGlob$1 && parseGlob ) || parseGlob$1;

var require$$11 = ( regexCache$1 && regexCache_1 ) || regexCache$1;
...
utils.diff = require$$0;
utils.unique = require$$1$1;
utils.braces = require$$2;
utils.brackets = require$$3;
utils.extglob = require$$4;
utils.isExtglob = extglob;
utils.isGlob = isglob;
utils.typeOf = typeOf;
utils.normalize = require$$8;
utils.omit = require$$9;
utils.parseGlob = require$$10;
utils.cache = require$$11;
...
var utils$3 = /*#__PURE__*/Object.freeze({
    default: utils_1,
    __moduleExports: utils_1
});

var chars$1 = {}, unesc, temp;
...
chars$1.UNESC = unesc || (unesc = reverse(chars$1.ESC, '\\'));
...
chars$1.TEMP = temp || (temp = reverse(chars$1.ESC_TEMP));

var chars_1 = chars$1;

var chars$2 = /*#__PURE__*/Object.freeze({
    default: chars_1,
    __moduleExports: chars_1
});

var chars$3 = ( chars$2 && chars_1 ) || chars$2;

var utils$4 = ( utils$3 && utils_1 ) || utils$3;
...
Glob.prototype.braces = function() {
  if (this.options.nobraces !== true && this.options.nobrace !== true) {
    ...
    var expanded = utils$4.braces(this.pattern, this.options);
...
Glob.prototype.brackets = function() {
  if (this.options.nobrackets !== true) {
    this.pattern = utils$4.brackets(this.pattern);
  }
};
...
Glob.prototype.extglob = function() {
  ...
  if (utils$4.isExtglob(this.pattern)) {
    this.pattern = utils$4.extglob(this.pattern, {escape: true});
  }
};
...
Glob.prototype.parse = function(pattern) {
  this.tokens = utils$4.parseGlob(pattern || this.pattern, true);
  return this.tokens;
};
...
var glob$1 = /*#__PURE__*/Object.freeze({
    default: glob,
    __moduleExports: glob
});

var Glob = ( glob$1 && glob ) || glob$1;
...
var expand$2 = /*#__PURE__*/Object.freeze({
    default: expand_1,
    __moduleExports: expand_1
});

var expand$3 = ( expand$2 && expand_1 ) || expand$2;
...
function match(files, pattern, opts) {
  if (utils$4.typeOf(files) !== 'string' && !Array.isArray(files)) {
    throw new Error(msg('match', 'files', 'a string or array'));
  }

  files = utils$4.arrayify(files);
  opts = opts || {};
  ...
  while (i < len) {
    var file = files[i++];
    var fp = utils$4.unixify(file, opts);
  ...
  if (res.length === 0) {
    ...
    if (opts.nonull || opts.nullglob) {
      res.push(utils$4.unescapeGlob(orig));
    }
  }

  // if `negate` was defined, diff negated files
  if (negate) { res = utils$4.diff(files, res); }

  // if `ignore` was defined, diff ignored filed
  if (opts.ignore && opts.ignore.length) {
    pattern = opts.ignore;
    opts = utils$4.omit(opts, ['ignore']);
    res = utils$4.diff(res, micromatch(res, pattern, opts));
  }

  if (opts.nodupes) {
    return utils$4.unique(res);
  }
  return res;
}
...
function filter$1(patterns, opts) {
  ...
  patterns = utils$4.arrayify(patterns);
  var len = patterns.length, i = 0;
...
function isMatch(fp, pattern, opts) {
  ...
  fp = utils$4.unixify(fp, opts);
  if (utils$4.typeOf(pattern) === 'object') {
...
function contains(fp, pattern, opts) {
  ...
  if (opts.contains && !utils$4.isGlob(pattern)) {
...
function matcher(pattern, opts) {
  ...
  pattern = utils$4.unixify(pattern, opts);

  // pattern is a non-glob string
  if (!utils$4.isGlob(pattern)) {
    return utils$4.matchPath(pattern, opts);
  }
  ...
  if (opts && opts.matchBase) {
    return utils$4.hasFilename(re, opts);
  }
...
function makeRe(glob, opts) {
  if (utils$4.typeOf(glob) !== 'string') {
    throw new Error(msg('makeRe', 'glob', 'a string'));
  }
  return utils$4.cache(toRegex$1, glob, opts);
}
...
/* eslint no-multi-spaces: 0 */
micromatch.any       = any;
micromatch.braces    = micromatch.braceExpand = utils$4.braces;
micromatch.contains  = contains;
micromatch.expand    = expand$3;
