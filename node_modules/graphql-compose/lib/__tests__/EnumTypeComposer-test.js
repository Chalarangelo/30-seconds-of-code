...
describe('EnumTypeComposer', () => {
  ...
  let etc;
  beforeEach(() => {
    enumType = new _graphql.GraphQLEnumType({
      name: 'MyEnum',
      values: {
        VAL1: {
          value: 'VAL1'
    ...
    etc = new _EnumTypeComposer.EnumTypeComposer(enumType, _.schemaComposer);
  });
  describe('values manipulation', () => {
    ...
    if (_graphqlVersion.graphqlVersion >= 13) {
      it('getFields() from empty Enum', () => {
        const etc2 = _EnumTypeComposer.EnumTypeComposer.create('SomeType', _.schemaComposer);
    ...
    describe('setFields()', () => {
      it('should set fields with standart config', () => {
        ...
        const valueConfig = enumType.getValue('VAL3');
    ...
    it('addFields()', () => {
      ...
      const valueConfig = enumType.getValue('VAL4');
    ...
    describe('extendField()', () => {
      ...
      it('should throw error if field does not exists', () => {
        expect(() => etc.extendField('unexisted', {
          description: '123'
  ...
  describe('create() [static method]', () => {
    if (_graphqlVersion.graphqlVersion >= 13) {
      it('should create ETC by typeName as a string', () => {
        const myTC = _EnumTypeComposer.EnumTypeComposer.create('TypeStub', _.schemaComposer);
    ...
    it('should create ETC by type template string', () => {
      const myTC = _EnumTypeComposer.EnumTypeComposer.create('enum SDLEnum { V1 V2 V3 }', _.schemaComposer);
    ...
    it('should create TC by GraphQLEnumType', () => {
      ...
      const myTC = _EnumTypeComposer.EnumTypeComposer.create(objType, _.schemaComposer);
    ...
    it('should create TC without values from string', () => {
      const myTC = _EnumTypeComposer.EnumTypeComposer.create('MyEnum', _.schemaComposer);

      expect(myTC.getFieldNames()).toEqual([]);
    });
    it('should create type and store it in schemaComposer', () => {
      const SomeUserETC = _EnumTypeComposer.EnumTypeComposer.create('SomeUserEnum', _.schemaComposer);

      expect(_.schemaComposer.getETC('SomeUserEnum')).toBe(SomeUserETC);
    });
    it('createTemp() should not store type in schemaComposer', () => {
      _EnumTypeComposer.EnumTypeComposer.createTemp('SomeUserEnum');

      expect(_.schemaComposer.has('SomeUserEnum')).toBeFalsy();
    });
  });
  describe('type methods', () => {
    ...
    it('getTypePlural() should return wrapped type with GraphQLList', () => {
      expect(etc.getTypePlural()).toBeInstanceOf(_graphql.GraphQLList);
      expect(etc.getTypePlural().ofType).toBe(etc.getType());
    });
    it('getTypeNonNull() should return wrapped type with GraphQLNonNull', () => {
      expect(etc.getTypeNonNull()).toBeInstanceOf(_graphql.GraphQLNonNull);
  ...
  describe('directive methods', () => {
    it('type level directive methods', () => {
      const tc1 = _.schemaComposer.createEnumTC(`
  ...
  describe('merge()', () => {
    it('should merge with GraphQLEnumType', () => {
      const sortETC = _.schemaComposer.createEnumTC(`enum Sort { ID_ASC ID_DESC }`);
    ...
    it('should throw error on wrong type', () => {
      ...
      expect(() => sortETC.merge(_.schemaComposer.createScalarTC('Scalar'))).toThrow('Cannot merge ScalarTypeComposer');
    });
  });
});