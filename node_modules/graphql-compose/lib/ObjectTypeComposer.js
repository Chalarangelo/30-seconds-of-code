...
function isComposeOutputType(type) {
  return (0, _graphql.isOutputType)(type) || Array.isArray(type) && isComposeOutputType(type[0]) || type instanceof ObjectTypeComposer || type instanceof _InterfaceTypeComposer.InterfaceTypeComposer || type instanceof _EnumTypeComposer.EnumTypeComposer || type instanceof _UnionTypeComposer.UnionTypeComposer || type instanceof _ScalarTypeComposer.ScalarTypeComposer || type instanceof _Resolver.Resolver;
} // Compose Args -----------------------------
...
class ObjectTypeComposer {
  ...
  static createTemp(typeDef, schemaComposer) {
    ...
    if ((0, _is.isString)(typeDef)) {
      ...
      if (NAME_RX.test(typeName)) {
        TC = new ObjectTypeComposer(new _graphql.GraphQLObjectType({
          name: typeName,
          fields: () => ({})
        }), sc);
      } else {
    ...
    } else if (typeDef instanceof _graphql.GraphQLObjectType) {
      TC = new ObjectTypeComposer(typeDef, sc);
    } else if ((0, _is.isObject)(typeDef)) {
      const fields = typeDef.fields;
      const interfaces = typeDef.interfaces;
      const type = new _graphql.GraphQLObjectType(_objectSpread({}, typeDef, {
        fields: (0, _is.isFunction)(fields) ? () => (0, _configAsThunk.resolveOutputConfigMapAsThunk)(sc, fields(), typeDef.name) : () => ({}),
        interfaces: (0, _is.isFunction)(interfaces) ? () => (0, _configAsThunk.resolveInterfaceArrayAsThunk)(sc, interfaces, typeDef.name) : []
      }));
    ...
    Object.keys(newFields).forEach(fieldName => {
      ...
      if (names.length === 0) {
        ...
        if (!this.hasField(name)) {
          childTC = ObjectTypeComposer.createTemp(`${this.getTypeName()}${(0, _misc.upperFirst)(name)}`, this.schemaComposer);
          this.setField(name, {
            type: childTC,
            resolve: () => ({})
          });
  ...
  getField(fieldName) {
    ...
    if ((0, _is.isFunction)(field)) field = field();
  ...
  getFieldArgType(fieldName, argName) {
    ...
    const graphqlAC = (0, _configAsThunk.resolveArgConfigAsThunk)(this.schemaComposer, ac, argName, fieldName, this.getTypeName());
    return graphqlAC.type;
  }

  getFieldArgTC(fieldName, argName) {
    const fieldType = (0, _graphql.getNamedType)(this.getFieldArgType(fieldName, argName));
    const tc = this.schemaComposer.createTC(fieldType);

    if (tc instanceof ObjectTypeComposer || tc instanceof _InterfaceTypeComposer.InterfaceTypeComposer || tc instanceof _UnionTypeComposer.UnionTypeComposer) {
  ...
  clone(newTypeName) {
    ...
    const cloned = new ObjectTypeComposer(new _graphql.GraphQLObjectType({
      name: newTypeName,
      fields: newFields
    }), this.schemaComposer);
  ...
  merge(type) {
    if (type instanceof _graphql.GraphQLObjectType) {
      this.addFields((0, _configToDefine.defineFieldMapToConfig)(type.getFields()));
    ...
    } else if (type instanceof _InterfaceTypeComposer.InterfaceTypeComposer) {
      this.addFields(type.getFields());
    } else {
      throw new Error(`Cannot merge ${(0, _misc.inspect)(type)} with ObjectType(${this.getTypeName()}). Provided type should be GraphQLObjectType or ObjectTypeComposer.`);
    }
  ...
  addResolver(opts) {
    ...
    if (!(opts instanceof _Resolver.Resolver)) {
      ...
      resolver = new _Resolver.Resolver(resolverOpts, this.schemaComposer);
    } else {
      resolver = opts;
    }
  ...
  getInterfaces() {
    if (!this.gqType._gqcInterfaces) {
      ...
      this.gqType._gqcInterfaces = (0, _configAsThunk.resolveInterfaceArrayAsThunk)(this.schemaComposer, interfaces, this.getTypeName()) || [];
    }
  ...
  hasInterface(iface) {
    const nameAsString = (0, _typeHelpers.getComposeTypeName)(iface);
    const ifaces = this.getInterfaces();
    return !!ifaces.find(i => (0, _typeHelpers.getComposeTypeName)(i) === nameAsString);
  }
