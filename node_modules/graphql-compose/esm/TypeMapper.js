...
export class TypeMapper {
  ...
  initScalars() {
    const scalars = [['String', GraphQLString], ['Float', GraphQLFloat], ['Int', GraphQLInt], ['Boolean', GraphQLBoolean], ['ID', GraphQLID], ['JSON', GraphQLJSON], ['Json', GraphQLJSON], ['Date', GraphQLDate], ['Buffer', GraphQLBuffer]];
  ...
  convertOutputFieldConfig(composeFC, fieldName = '', typeName = '') {
    ...
    if (composeFC instanceof GraphQLList || composeFC instanceof GraphQLNonNull) {
      ...
      const _ref = composeFC,
            type = _ref.type,
            args = _ref.args,
            rest = _objectWithoutPropertiesLoose(_ref, ["type", "args"]);
    ...
    if (!isFunction(fieldConfig.type)) {
      if (!isOutputType(fieldConfig.type)) {
        throw new Error(`${typeName}.${fieldName} provided incorrect OutputType: '${inspect(composeType)}'`);
      }
  ...
  convertArgConfig(composeAC, argName = '', fieldName = '', typeName = '') {
    ...
    if (composeAC instanceof GraphQLList || composeAC instanceof GraphQLNonNull) {
      ...
      const _ref2 = composeAC,
            type = _ref2.type,
            rest = _objectWithoutPropertiesLoose(_ref2, ["type"]);
    ...
    if (!isFunction(argConfig.type)) {
      if (!isInputType(argConfig.type)) {
        throw new Error(`${typeName}.${fieldName}@${argName} provided incorrect InputType: '${inspect(composeType)}'`);
      }
  ...
  convertInputFieldConfig(composeIFC, fieldName = '', typeName = '') {
    ...
    if (composeIFC instanceof GraphQLList || composeIFC instanceof GraphQLNonNull) {
      ...
      const _ref3 = composeIFC,
            type = _ref3.type,
            rest = _objectWithoutPropertiesLoose(_ref3, ["type"]);
  ...
  convertInterfaceType(composeType) {
    ...
    throw new Error(`Cannot convert to InterfaceType the following object: '${inspect(composeType)}'`);
  }
    ...
    values.forEach(value => {
      ...
      if (value.defaultValue) {
        ...
        if (isInputType(wrappedType)) {
          ac.defaultValue = valueFromAST(value.defaultValue, wrappedType);
        } else {
  ...
  getDeprecationReason(directives) {
    ...
    const _getArgumentValues = getArgumentValues(GraphQLDeprecatedDirective, deprecatedAST),
